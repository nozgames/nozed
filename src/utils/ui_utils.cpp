                        ...
                        Row({.spacing=20}, [] {
                            GestureDetector({.on_tap=HandleYes}, [] {
                                Container({.width=100, .height=24}, [] {
                                    ...
                                    Label("YES", {.font = FONT_SEGUISB, .font_size=18, .color=COLOR_UI_BUTTON_TEXT, .align = ALIGNMENT_CENTER});
                                });
                            });
                            GestureDetector({.on_tap=HandleNo}, [] {
                                Container({.width=100, .height=24}, [] {
                                    Rectangle({.color_func=GetButtonHoverColor});
                                    Label("NO", {.font = FONT_SEGUISB, .font_size=18, .color=COLOR_UI_BUTTON_TEXT, .align = ALIGNMENT_CENTER});
                                });
                            });
                        });
                    });
