...
{
    BeginCanvas();
    BeginElement(STYLE_NOTIFICATIONS_NOTIFICATION_CONTAINER);
        for (int i=0, c=g_notifications.buffer->count; i<c; i++)
        {
            ...
            BeginElement(STYLE_NOTIFICATIONS_NOTIFICATION);
                Label(n->text, STYLE_NOTIFICATIONS_NOTIFICATION_TEXT);
            EndElement();
        }
    EndElement();
    EndCanvas();
